...
extract_data_files() {
    ...
    # Look for lines with "filename" or the hospital data file and extract the value
    grep -E "(filename|hospital_data_file).*=" "$input_file" | \
        awk -F'=' '{print $2}' | \
        tr -d ' "'"'"'' | \
        grep -v "^output" | \
        sort -u
...
process_ensemble_case() {
    ...
    if [[ "$dry_run" == "true" ]]; then
        ...
        case "$platform" in
            ...
            *)
                local jobid
                jobid=$( (cd "$ensemble_dir" && sbatch exaepi.job) | awk '{print $NF}')
                echo "$jobid" > "${ensemble_dir}/job_id.txt"
                print_success "Ensemble job submitted for ${case_name} (job ${jobid})!"
                ;;
        esac
    fi
